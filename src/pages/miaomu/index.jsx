...
const Content = (props) => {
  ...
  const checkDataLength = (list) => {
    let status;
    if (list.data && list.data.length > 4) {
      if (list.show) {
        status = false;
      } else {
        status = true;
  ...
  return (
    <div>
      {isLoading ?
        ...
        :
        showInfo ?
          <Detail isMaiomu detailObj={{ images: [], head: detail.title, text: detail.text }}
            video={detail.video && detail.video.path} />
          :
              ...
              contentList.map((list, index) => {
                return (
                  <div key={index} style={{ position: 'relative' }}>
                    <h1 styleName="list-head">{list.parent}</h1>
                    {list.listData.map((child, childIndex) => {
                      return (
                        <div key={childIndex}>
                          <h3 styleName="list-title">{child.title}</h3>
                          {checkDataLength(child) ? <Button styleName="more-btn body-button" onClick={() => {
                            handleShowAll(child, childIndex)
                          }}>查看更多</Button> : ''}
                          <div styleName="list">
                            {
...
class MiaoMu extends Component {
  ...
  getContentList(id, title) {
    ...
    getList({ id }).then(req => {
      this.setState({ listLoading: false });
      this.setState({ listData: [{ parent: '', listData: [{ data: req || [], title, show: true }] }] })
    }).catch(_ => {
      console.log(_);
      this.setState({ listLoading: false });
    })
        ...
        <Hideen smDown>
          <div styleName="menu">
            <div styleName="img-group">
              ...
              <img style={{ cursor: 'pointer' }} src={require('../../assets/images/icont_tip_bg2.png')} alt=""
                onClick={() => {
                  this.getAllContentList()
                }} />
            </div>
